...
static void preparePage(void) {
    ...
    if (G_context.auth.function_type == SOROBAN_AUTHORIZED_FUNCTION_TYPE_CREATE_CONTRACT_HOST_FN) {
        ...
        strlcpy(str_values[0], "Create Smart Contract", DETAIL_VALUE_MAX_LENGTH);
        ...
        if (!print_sc_address(&G_context.auth.invoke_contract_args.address,
                              str_values[0],
                              DETAIL_VALUE_MAX_LENGTH,
                              0,
                              0)) {
        ...
        strlcpy(str_values[1],
                G_context.auth.invoke_contract_args.function_name.text,
                DETAIL_VALUE_MAX_LENGTH);
        caption_value_pairs[1].value = str_values[1];
        ...
        if (!print_uint(G_context.auth.nonce, str_values[3], DETAIL_VALUE_MAX_LENGTH)) {
            io_send_sw(SW_DISPLAY_ADDRESS_FAIL);
            return;
        }
        ...
        if (!print_uint(G_context.auth.signature_exp_ledger,
                        str_values[4],
                        DETAIL_VALUE_MAX_LENGTH)) {
            io_send_sw(SW_DISPLAY_ADDRESS_FAIL);
            return;
        ...
        switch (G_context.auth.invoke_contract_args.contract_type) {
            case SOROBAN_CONTRACT_TYPE_UNVERIFIED:
                ...
                if (!format_hex(G_context.hash, 32, str_values[5], DETAIL_VALUE_MAX_LENGTH)) {
            ...
            case SOROBAN_CONTRACT_TYPE_ASSET_APPROVE:
                ...
                if (!print_sc_address(&G_context.auth.invoke_contract_args.asset_approve.spender,
                                      str_values[5],
                                      DETAIL_VALUE_MAX_LENGTH,
                                      0,
                                      0)) {
                ...
                if (!print_sc_address(&G_context.auth.invoke_contract_args.asset_approve.from,
                                      str_values[6],
                                      DETAIL_VALUE_MAX_LENGTH,
                                      0,
                                      0)) {
            ...
            case SOROBAN_CONTRACT_TYPE_ASSET_TRANSFER:
                ...
                caption_value_pairs[6].item = "From";
                if (!print_sc_address(&G_context.auth.invoke_contract_args.asset_transfer.from,
                                      str_values[6],
                                      DETAIL_VALUE_MAX_LENGTH,
                                      0,
                                      0)) {
                ...
                if (!print_sc_address(&G_context.auth.invoke_contract_args.asset_transfer.to,
                                      str_values[7],
                                      DETAIL_VALUE_MAX_LENGTH,
                                      0,
                                      0)) {
